...
Show that the least squares estimator $\hat{\beta} = (X^TX)^{-1}X^TY$ is an unbiased estimator.
...
For each of the following regression models, indicate if it is a linear regression model. Justify your answer.
...
Given the analysis in the previous plots, will it be appropriate to transform the predictors or the response or both? Why? If both, then which should you transform first?
...
Make diagnostic plots to verify if the Box-Cox model developed in the previous question satisfies the 3 assumptions mentioned in the first question. Is the Box-Cox model an improvement over the previous model with regard to the assumptions? Explain.

*(4 points)*

### 

Plot the residuals against each predictor for the Box-Cox model. For each predictor, comment if it seems to have a linear relationship with the response, and if the error variance seems constant.

*(8 points)*

### 

Based on the above plots, transform two predictor to improve the model on the linear relationship assumption with regard to the predictors. Mention the intuition behind the transformation.

*(4 points)*


Make the diagnostic plots, and comment if the model has improved over the Box-Cox model with regard to the 3 model assumptions and goodness-of-fit, and if it seems to satisfy the 3 assumptions.
