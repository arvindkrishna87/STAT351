...
Estimate the expected decrease in number of crimes per 100,000 residents when the number of individuals having high school diploma increases by 1\%. Use a 99\% confidence interval. Interpret the interval estimate.

*(4 + 2 = 6 points)*

### 

For what percentage of number of individuals having high school diploma will you be the most confident in estimating the crime rate with a confidence interval of a given width?

*(2 points)*

### 

For what range of values of the percentage of number of individuals having high school diploma will it be inappropriate to use the developed model to predict the crime rate?

*(2 points)*

### 

Predict the number of crimes per 100,000 residents of a county using a 90\% prediction interval if 75\% of the individuals have a high school diploma. Interpret the prediction interval.

*(2 + 2 = 4 points)*

###

A consultant had stated that the number of crimes per 100,000 should reduce by at least 1000 for 4\% increase in the proportion of people with high school diplomas. Conduct a hypothesis test using the probability of type 1 error as 5\%. State the null and alternate hypotheses, the test, and conclusion from the test.
...
Consider the following data and the developed model. 
...
Execute the code to obtain the model summary. You will find that the intercept is statistically insignificant. As it is insignificant, a student suggested that it won't matter if it is removed from the developed model, i.e., the developed model is changed to:
...
Use the data to compute the value of the bias obtained from the expression derived in the previous question.

*(2 points)*

### 

Now you will use simulations to verify the value of bias computed in the previous question. Simulate the data 1000 times. In each iteration, estimate $\beta_1$. Report the bias in the estimate of $\beta_1$ based on the 1000 simulations. Is it the same as obtained analytically in the previous question?

*(5 + 1 = 6 points)*

### 

Plot the 1000 regression lines based on the 1000 simulations over a scatterplot of the data points.

*(2 points)*

### 

Now consider the usual simple linear regression model with the intercept. Simulate the data 1000 times. In each iteration, estimate $\beta_1$. Report the bias in the estimate of $\beta_1$ based on the 1000 simulations. 
...
If the intercept is statistically insignificant, should it be removed from the model, and model (1) considered instead? Explain.
...
In terms of bias and variance, mention an advantage and disadvantage of the estimate of $\beta_1$ from model (2) against that obtained from model (1). The plots developed in earlier questions may also help visualize the advantages and disadvantages.
