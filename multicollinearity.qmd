...
# Developing model
model <- lm(Balance~., data = credit_data)
summary(model)
```
...
Note that the VIF of `Income` and `Rating` is very high indicating presence of multicollinearity. This may have lead to a high standard error for the coefficients of these predictors.

Let us remove `Rating` from the model as it seems to be redundant based on high VIF.

```{r}
model <- lm(Balance~. - Rating, data = credit_data)
summary(model)
```

Note that the $R^2_{adj}$ of the updated model is almost the same as that of the previous model. Also, note that the standard error of the estimated coefficient of `Income` has reduced resulting in a more precise estimate.

Let us re-check the VIF of the predictors of the updated model.

```{r}
vif(model)
```

The model no longer suffers from multicollinearity.
